...
public class UDPConnTestClient extends Thread {
	...
	private static final int[] test_msizes =
		new int[]{ 5000, 10000, 15000, 20000, 25000, 30000, 35000};
		//new int[]{ 64, 256, 1024, 4096, 8192, 16384};
	private static final int test_mcount = 1024;
	//test results
	...
	public void close(){
		//request that reply thread stops
		reply_thread.request_stop();
		//wait for reply thread to stop
	...
	public void reply_wait(){
		...
		int staleness = 0;
		int wait_time = 100;
		int max_staleness = 24000 / wait_time;
		System.out.printf( "reply count: %d", reply_count);
		while( staleness < max_staleness){
			try{ Thread.sleep( wait_time);}
			catch( InterruptedException exception){}
			if( old_count < reply_count){
				System.out.printf( "\rreply count: %d", reply_count);
